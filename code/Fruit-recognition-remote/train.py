...
def inference(input_tensor, train, regularizer):
    ...
    with tf.compat.v1.variable_scope('layer1-conv1'):
        conv1_weights = tf.compat.v1.get_variable("weight", [5, 5, 3, 64],initializer=tf.truncated_normal_initializer(stddev=0.1))
        variable_summaries(conv1_weights)
        conv1_biases = tf.compat.v1.get_variable("bias", [64], initializer=tf.constant_initializer(0.0))
        variable_summaries(conv1_biases)
        conv1 = tf.nn.conv2d(input_tensor, conv1_weights, strides=[1, 1, 1, 1], padding='SAME')
        relu1 = tf.nn.relu(tf.nn.bias_add(conv1, conv1_biases))
        tf.compat.v1.summary.histogram('relu1', relu1)

    # 第二层池化层，输入100 * 100 * 64，输出50 * 50 * 64
    with tf.name_scope("layer2-pool1"):
        pool1 = tf.nn.max_pool2d(relu1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding="VALID")
        tf.compat.v1.summary.histogram('pool1', pool1)

    # 第三层卷积层，输入50 * 50 * 64，输出50 * 50 * 128
    with tf.compat.v1.variable_scope("layer3-conv2"):
        conv2_weights = tf.compat.v1.get_variable("weight", [5, 5, 64, 128], initializer=tf.truncated_normal_initializer(stddev=0.1))
        variable_summaries(conv2_weights)
        conv2_biases = tf.compat.v1.get_variable("bias", [128], initializer=tf.constant_initializer(0.0))
        variable_summaries(conv2_biases)
        conv2 = tf.nn.conv2d(pool1, conv2_weights, strides=[1, 1, 1, 1], padding='SAME')
        relu2 = tf.nn.relu(tf.nn.bias_add(conv2, conv2_biases))
        tf.compat.v1.summary.histogram('relu2', relu2)
    ...
    with tf.variable_scope('layer7-fc3'):
        ...
        if regularizer is not None:
            tf.compat.v1.add_to_collection('losses', regularizer(fc3_weights))
...
optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=0.001).minimize(cross_entropy)
...
writer = tf.compat.v1.summary.FileWriter(log_path, sess.graph)
sess.run(tf.compat.v1.global_variables_initializer())
for i in range(epoch):
    # 训练集
    train_loss, train_acc, n_batch = 0, 0, 0
    for x_train_a, y_train_a in get_feed_data(x_train, y_train, batch_size, shuffle=True):
        summary, err, ac = sess.run([optimizer, cross_entropy, accuracy], feed_dict={x: x_train_a, y: y_train_a})
        train_loss += err
        train_acc += ac
        n_batch += 1
    ...
    if np.sum(val_acc) / n_batch >= 0.97:
        break
